# 📱 GUIDE D'UTILISATION — GIRASOLE DIAGPV

> **Application d'audits photovoltaïques terrain — 100% serverless Cloudflare**
...
- ✅ **Audit TOITURE** : Checklist 13 points (si `audit_toiture = 'X'`)
...
### **2️⃣ Pendant la mission (sur terrain)**
1. **Checklist SOL** : 40 points de contrôle
   - Cliquer sur chaque bouton : ✅ CONFORME / ❌ NON CONFORME / ➖ N/A
   - Ajouter des photos par item (multi-upload)
   - Ajouter des commentaires si nécessaire
   
2. **Checklist TOITURE** (si applicable) : 13 points de contrôle
   - **Nouveaux items** (27/11/2024) :
     - Point 12 : Présence de film anticondensation ?
     - Point 13 : Présence de limaille/rouille/piquage sous modules ?
   - Photos + commentaires par item
   
3. **Photos générales** : Galerie globale de la centrale
   - Ajouter légendes descriptives

4. **Commentaire final** : Synthèse de l'audit
   - Auto-sauvegarde toutes les 2 secondes

### **3️⃣ Après la mission**
1. Cliquer sur **"📄 GÉNÉRER RAPPORT HTML/PDF"**
...
| Type de données | Table DB | Localisation | Capacité |
|----------------|----------|--------------|----------|
| **Centrales** | `centrales` | Cloudflare D1 | 52 installations |
| **Missions** | `ordres_mission` | Cloudflare D1 | 52 missions |
| **Checklist SOL** | `checklist_items` | Cloudflare D1 | 2080 items (52×40) |
...
| **Photos générales** | `ordres_mission_photos_generales` | Cloudflare D1 | Base64 complet |
| **Commentaires item** | `checklist_items.commentaire` | Cloudflare D1 | Texte illimité |
| **Commentaire final** | `ordres_mission_commentaires_finaux` | Cloudflare D1 | Texte illimité |
| **Rapports HTML/PDF** | `rapports_audits` | Cloudflare D1 | JSON structure |

### **🔒 Sécurité & Sauvegarde**

✅ **0% localStorage** : Aucune donnée dans le navigateur  
✅ **100% Cloudflare D1** : Base de données distribuée globalement  
✅ **Auto-sauvegarde** : Chaque clic = sauvegarde immédiate en DB  
✅ **Photos base64** : Stockées directement en DB (pas de fichiers)  
✅ **Commentaires auto-save** : Débounce 2 secondes  
✅ **Hors-ligne** : Impossible (nécessite connexion internet)  

### **🌍 Infrastructure technique**

- **Cloudflare Workers** : Edge computing (latence < 50ms)
- **Cloudflare D1** : SQLite distribué (multi-régions)
...
|----------|---------|-------------|
| `/api/checklist/:mission_id` | GET | Récupérer checklist SOL (40 items) |
| `/api/checklist-toiture/:mission_id` | GET | Récupérer checklist TOITURE (13 items) |
...
| `/api/ordres-mission/:mission_id/photos-generales` | POST | Uploader photos générales |
| `/api/ordres-mission/:mission_id/commentaire-final` | PUT | Sauvegarder commentaire final |
| `/api/rapports/generer/:mission_id` | POST | Générer rapport HTML/PDF |
...
Systématique pour toutes les missions. Points de contrôle :
1. Présence autocontrôle renseigné installateur
2. Fixations chemin câbles (qualité + méthode)
3. Type cheminement câbles (galvanisé chaud/froid)
4. Protections mécaniques câbles (contacts saillants)
5. Repérage équipements (inscrit sur équipement)
6. Type repérage câbles (sérigraphié/manuscrit)
7. Connecteurs MC4 serrés/étanches
8. État visuel modules (fissures/casse)
9. État visuel jonctions coffrets
...
7. Qualité cheminement câbles DC
...
⚠️ **Internet requis** : Pas de mode hors-ligne
