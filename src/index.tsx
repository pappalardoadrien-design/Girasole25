...
// =======================
// RAPPORTS AUDITS - APIs ONLY
// =======================

// POST /api/rapports/generer/:mission_id - Générer rapport factuel
  ...
  try {
    ...
    const rapportData = {
      ...
      checklist_sol: {
        ...
        stats: { conformes: nbConformesSol, non_conformes: nbNonConformesSol, na: nbNaSol, total: itemsSol.results.length }
      },
    ...
    return c.json({ success: true, rapport_id: insertRapport.meta.last_row_id, message: 'Rapport généré (factuel uniquement)' })
  } catch (error) {
    return c.json({ success: false, error: String(error) }, 500)
  }
})
...
app.get('/api/rapports', async (c) => {
  ...
  try {
    const rapports = await DB.prepare(`
      SELECT r.*, c.nom as centrale_nom, c.type_centrale, c.puissance_kwc
...
app.get('/api/rapports/:rapport_id', async (c) => {
  ...
  try {
    const rapport = await DB.prepare(`
      SELECT r.*, c.nom as centrale_nom, c.type_centrale, c.puissance_kwc, c.localisation
      FROM rapports_audits r
      JOIN centrales c ON r.centrale_id = c.id
      WHERE r.id = ?
    `).bind(rapportId).first()
    if (!rapport) return c.json({ success: false, error: 'Rapport non trouvé' }, 404)
...
app.post('/api/rapports/:rapport_id/complements', async (c) => {
  ...
  try {
    ...
    const result = await DB.prepare(`
      INSERT INTO rapports_complements (rapport_id, type, contenu, titre, description, auteur, section_rapport)
      VALUES (?, ?, ?, ?, ?, ?, ?)
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Migration Audits GIRASOLE</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif;
...
<body>
    <div class="container">
        ...
        <div class="subtitle">Récupération de tes 9 audits terrain</div>
        ...
        <div id="instructions">
            <div class="step">
                ...
                <div class="step-desc">
                    Tu as rempli 9 checklists sur cette URL (iPhone Chrome) :
                    <div class="url-box">0da64170.girasole-diagpv.pages.dev</div>
                    Les données sont dans le <strong>localStorage de ton navigateur</strong>.
                </div>
            </div>
            ...
            <div class="step">
                ...
                <div class="step-desc">
                    L'ancienne version écrasait les données à chaque refresh. 
                    La nouvelle version (0c2f94b2) corrige ça, mais elle ne peut pas accéder 
                    aux données de l'ancienne URL.
                </div>
            </div>
            ...
            <div class="step">
                ...
                <div class="step-desc">
                    Cette page va <strong>exporter automatiquement</strong> tes 9 audits 
                    depuis le localStorage vers un fichier JSON.
                </div>
            </div>

            <div class="warning">
                ⚠️ <strong>IMPORTANT</strong> : Cette page doit être ouverte sur le 
                <strong>même iPhone Chrome</strong> que celui utilisé pour remplir les audits.
            </div>

            <button id="btn-migrate" class="btn btn-primary">
                🚀 EXPORTER MES 9 AUDITS
            </button>
        </div>
        ...
        <div id="loading" style="display:none;">
            <div class="loading">
                ...
                <p>Analyse du localStorage en cours...</p>
            </div>
        </div>

        <div id="success" style="display:none;">
            <div class="success">
                <h2 style="margin-bottom:10px;">✅ Export réussi !</h2>
            ...
            setTimeout(function() {
                try {
                    ...
                    MISSIONS.forEach(mission => {
                        ...
                        if (data) {
                            try {
                                ...
                                // Photos générales
                                const photosGenerales = JSON.parse(localStorage.getItem('photos_generales_' + mission.id) || '[]');
                                totalPhotos += photosGenerales.length;
                                ...
                                console.error('Erreur mission ' + mission.id, e);
                            }
                        }
                    });
                    ...
                    if (foundCount > 0) {
                        ...
                        const a = document.createElement('a');
                        a.href = url;
                        a.download = 'audits_GIRASOLE_' + foundCount + 'missions_' + new Date().toISOString().split('T')[0] + '.json';
                        document.body.appendChild(a);
                        a.click();
                        document.body.removeChild(a);
                        URL.revokeObjectURL(url);
                        ...
                        document.getElementById('success-message').innerHTML = \`
                            <strong>\${foundCount} audits exportés</strong><br>
                            \${totalItems} items de checklist<br>
                            \${totalPhotos} photos incluses<br><br>
                            📥 Fichier téléchargé !<br><br>
                            <strong>➡️ Envoie-moi ce fichier JSON</strong><br>
                            Je vais l'importer dans la DB en 2 min.
                        \`;
                    } else {
                        alert('❌ Aucun audit trouvé dans le localStorage.\\n\\nEs-tu sur le bon iPhone Chrome ?');
                        document.getElementById('loading').style.display = 'none';
...
// Route test API (debug cache) - Version JSON simple
app.get('/test-api', async (c) => {
  try {
    ...
    const missions = await DB.prepare(`
      ...
      JOIN centrales c ON om.centrale_id = c.id
      LEFT JOIN checklist_items ci ON om.id = ci.ordre_mission_id
      WHERE om.id IN (7, 12, 20, 24, 33, 45, 46)
      GROUP BY om.id
      ORDER BY om.id
    `).all()
    ...
    return c.html(`
      ...
      <head>
        <meta charset="UTF-8">
        <title>Test API DiagPV</title>
        <script src="https://cdn.tailwindcss.com"></script>
      </head>
      <body class="bg-gray-100 p-8">
        <div class="max-w-4xl mx-auto">
          <h1 class="text-3xl font-bold mb-6">🔍 Test API - Missions Critiques</h1>
          <div class="bg-white rounded-lg shadow p-6">
            <h2 class="text-xl font-bold mb-4">Résultats directs DB Cloudflare :</h2>
            ${missions.results.map((m: any) => {
              const progression = m.nb_points_total > 0 
                ? Math.round((m.nb_points_completes / m.nb_points_total) * 100) 
                : 0
              return `
                <div class="border-l-4 border-blue-500 pl-4 mb-4">
                  ...
                  <p class="text-sm text-gray-600">
                    <strong class="text-blue-600">${m.nb_points_completes}/${m.nb_points_total}</strong> points (${progression}%)
                  </p>
                  <div class="w-full bg-gray-200 rounded-full h-3 mt-2">
                    <div class="bg-blue-600 h-3 rounded-full" style="width: ${progression}%"></div>
                  </div>
                  <a href="/audit/${m.mission_id}" class="inline-block mt-2 bg-blue-600 text-white px-4 py-2 rounded text-sm">
                    📋 Ouvrir Checklist
                  </a>
                </div>
              `
